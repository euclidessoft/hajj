...
class DefaultController extends AbstractController
{
    ...
    */
    ...
    /**
     * @Route("/pelerinConfirmerPrint", name="Mecque_PelerinConfirmer_print")
     */
    public function pelerinConfirmerPrint(Request $request)
    {
		if($this->getUser() != null)
		{
			$em = $this->getDoctrine()->getManager();
			...
			if(!empty($session))
			{
				...
				$response = $this->render('Default/pelerinconfirmer_print.html.twig', array('pelerins' => $pelerins, 'session' => $session->getDesignation()));
				$response->setSharedMaxAge(0);
				$response->headers->addCacheControlDirective('no-cache', true);
				$response->headers->addCacheControlDirective('no-store', true);
				$response->headers->addCacheControlDirective('must-revalidate', true);
